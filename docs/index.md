---
title: Stream Watcher
---

# Stream Watcher

Windows desktop app for watching several Twitch streams at once. Download the EXE, open it, add channels. No Node, no wizard.

[How to use](USAGE.md) · [Feature list](FEATURES.md)

## Get the app

1. Open the latest [GitHub Release](https://github.com/Justin-Developer01/stream-watcher/releases).
2. Download **Stream Watcher Setup** (installer) or **Portable** (just run).
3. Windows SmartScreen may warn because the build is unsigned. **More info → Run anyway**.
4. You land on a thin chrome bar. The main window is frameless (no OS title bar) — drag the bar to move. Four short tips appear once (Focus mode, chat, Lock, Dock back).

In the app, **Settings → Help** is the short guide. Full docs links sit on that tab and under Advanced.

To publish these pages as a site: repo **Settings → Pages → Deploy from a branch → `/docs`**.

## Quick start

1. **Add a channel** — click the title (list icon if Chrome is Left).
2. **Login to Twitch** — log-in icon. Tooltip: **Login for Prime + chat.**
3. **Focus mode** — one stream large, others in the strip. **Switch Focus** promotes the next strip tile.

Then, for another monitor: park on another monitor → **Lock window (click-through)** when you need the UI → **Dock back** to the main desk.

| Do this | Where |
|---|---|
| Add a channel | Click the **title** in the chrome |
| Login for Prime + chat | Log-in icon. Tooltip: **Login for Prime + chat.** |
| Focus one stream | **Focus mode**. Click a strip tile, or **Switch Focus**. |
| Open chat (in the window) | **Open chat** on the thin bar, or **Open #channel chat** on a tile. Drawer **pushes** the grid. |
| Chat on another monitor | **Pop out chat** (bar, tile, or drawer). |
| Stream on another monitor | Tile **Pop out stream**. Tile reads **On another monitor**. |
| Dock / pin | **Dock back** and **Always on top** live on the pop-out only. Position is remembered. |
| Dock every pop-out | Reset icon. Tooltip: **Dock all pop-outs back**. |
| Save a layout | **Layout templates** popover: **+** saves, per-row delete removes. |
| Lock the UI | **Lock window (click-through)** after See desktop, when you need to click the desk. |
| Settings / Help | Gear. Tabs: Appearance · Chat · Hotkeys · Updates · Advanced · Help. Esc / X closes. |
| Chrome side | Settings → Appearance → **Chrome: Top \| Left** (Top default). |
| Ghost overlay | Settings → Appearance → **Ghost overlay**. Auto-hides the thin bar until the edge or Pin. Frameless is always on. |
| Chat type | Settings → **Chat**: Font (incl. **Custom…**), Size, Drawer width 280–480. Emotes after Login to Twitch. |
| Quit | Settings footer **Exit Stream Watcher**, or **Quit application** (Ctrl+Q). |
| Check for Updates | Settings → **Updates** |
| See the desktop | Settings → Appearance → **See desktop behind app.** |
| Save CPU | Performance gauge on the thin bar. Other tiles show **paused / low**. |

Full walkthrough: [USAGE.md](USAGE.md). Everything the app does: [FEATURES.md](FEATURES.md).
